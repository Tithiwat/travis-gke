...
yes | gcloud container images add-tag gcr.io/${PROJECT_NAME_STG}/${DOCKER_IMAGE_NAME}:$TRAVIS_COMMIT gcr.io/${PROJECT_NAME_STG}/${DOCKER_IMAGE_NAME}:latest

kubectl config view
gcloud container clusters get-credentials $CLUSTER_NAME_STG --zone $CLOUDSDK_COMPUTE_ZONE --project $PROJECT_NAME_STG
